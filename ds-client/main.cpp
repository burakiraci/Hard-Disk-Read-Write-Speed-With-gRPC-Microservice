...
//				std::cout << "Okuma Hizi: " << readSpeed << " MB/s" << std::endl;
//				std::cout << "Yazma Hizi: " << writeSpeed << " MB/s" << std::endl;
...
private:
	...
	void UpdateUI(double readSpeed, double writeSpeed) {
		// Arayüzü ana iş parçacığına yönlendirerek güncelle
...
public:
	DiskSpeedClient (std::shared_ptr<grpc::Channel> channel)
		: stub_ (diskspeed::DiskSpeedService::NewStub(channel)),
	...
	void MeasureSpeed(const std::string& file_name, int32_t file_size) {
		...
		while (true) {
			std::shared_ptr<grpc::Channel> channel = grpc::CreateChannel(server_address, grpc::InsecureChannelCredentials());
			std::unique_ptr<diskspeed::DiskSpeedService::Stub>
					stub(diskspeed::DiskSpeedService::NewStub(channel));
...
class UpdateSpeed : public QObject {
public:
	UpdateSpeed(Ui::Form* ui, Form* form)
		: QObject(nullptr), ui_(ui), form_(form)
	{}
...
int main(int argc, char* argv[]) {
	...
	std::string file_name = "test.txt";
	int32_t file_size = 1024 * 1024 * 100;
	UpdateSpeed update(w.getUI(), &w);
	app.installEventFilter(&update);
	std::thread grpcThread ([&]() {
		client.MeasureSpeed(file_name, file_size);
	});
	QTimer updateTimer;
	QObject::connect(&updateTimer, &QTimer::timeout, [&]() {
		...
		double writeSpeed = client.writeSpeed();
		QMetaObject::invokeMethod(qApp, [&, readSpeed, writeSpeed] () {
			update.event(new SpeedUpdateEvent(readSpeed, writeSpeed));
		});
	});
